...
struct vk_mat_vec_push_constants_addr {
...
static vk_buffer ggml_vk_create_buffer(vk_device& device, const vk::BufferCreateInfo& buffer_info, const VmaAllocationCreateInfo& alloc_info) {
    VK_LOG_DEBUG("ggml_vk_create_buffer(" << device->name << ", " << size << ")");
    ...
    for (uint32_t w = 0; w < DMMV_WG_SIZE_COUNT; ++w) {
        ...
        for (uint32_t i = 0; i < mul_mat_vec_max_cols; ++i) {
            ...
            if (device->integer_dot_product) {
                ...
                ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_q8_1_f32[w][GGML_TYPE_Q4_0][i], "mul_mat_vec_q4_0_q8_1_f32", arr_dmmv_q4_0_q8_1_f32_len[reduc], arr_dmmv_q4_0_q8_1_f32_data[reduc], "main", mul_mat_vec_num_bindings, sizeof(vk_mat_vec_push_constants_addr), {1*rm_stdq_int, 1, 1}, {wg_size_subgroup_int, 1*rm_stdq_int, i+1}, 1, true, use_subgroups, subgroup_size_int);
...
static vk_device ggml_vk_get_device(size_t idx) {
    ...
    if (vk_instance.devices[idx] == nullptr) {
        ...
        VmaAllocatorCreateFlags flags = (device->buffer_device_address ? VMA_ALLOCATOR_CREATE_BUFFER_DEVICE_ADDRESS_BIT : 0);
        ...
        vmaCreateAllocator(&allocatorCreateInfo, &device->allocator);
...
static void ggml_vk_ensure_sync_staging_buffer(ggml_backend_vk_context * ctx, size_t size) {
    if (ctx->sync_staging == nullptr || ctx->sync_staging->size < size) {
        ...
        VmaAllocationCreateInfo alloc_info = {};
        alloc_info.usage = (ctx->device->prefer_host_memory ? VMA_MEMORY_USAGE_AUTO_PREFER_HOST : VMA_MEMORY_USAGE_AUTO);
        alloc_info.flags = VMA_ALLOCATION_CREATE_HOST_ACCESS_RANDOM_BIT |
            VMA_ALLOCATION_CREATE_MAPPED_BIT;

        ctx->sync_staging = ggml_vk_create_buffer(ctx->device, buffer_info, alloc_info);
    }
}
...
static void ggml_vk_quantize_q8_1(ggml_backend_vk_context * ctx, vk_context& subctx, const vk_subbuffer & in, const vk_subbuffer & out, uint32_t ne) {
    ...
    const vk_quantize_q8_1_push_constants pc = {
            ne,
            static_cast<uint64_t>(in.buffer->bda_addr + in.offset),
            static_cast<uint64_t>(out.buffer->bda_addr + out.offset)
    };
...
static void ggml_vk_mul_mat_vec_q_f16(ggml_backend_vk_context * ctx, vk_context& subctx, const struct ggml_cgraph * cgraph, int node_idx) {
    ...
    if (dmmv->name == "mul_mat_vec_q4_0_q8_1_f32") {
        const vk_mat_vec_push_constants_addr pc = {
