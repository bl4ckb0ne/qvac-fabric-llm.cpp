...
void process_shaders() {
    ...
    for (const auto& tname : type_names) {
        ...
        if (tname == "q4_0") {
            string_to_spv("mul_mat_vec_" + tname + "_f32_f32", shader, merge_maps(base_dict, {{data_a_key, "1"}, {"B_TYPE", "float"}, {"B_TYPE_VEC2", "vec2"}, {"B_TYPE_VEC4", "vec4"}, {"D_TYPE", "float"}, {"USE_DEVICE_ADDR", "1"}}));
        } else {
        ...
        if (is_legacy_quant(tname) || tname == "mxfp4" || is_k_quant(tname)) {
            if (tname == "q4_0") {
                string_to_spv("mul_mat_vec_" + tname + "_q8_1_f32", "mul_mat_vecq.comp", merge_maps(base_dict, {{data_a_key, "1"}, {"D_TYPE", "float"}, {"FLOAT_TYPE", "float"}, {"FLOAT_TYPE_VEC2", "vec2"}, {"ACC_TYPE", "float"}, {"USE_DEVICE_ADDR", "1"}}));
            } else {
